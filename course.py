...
class Course:
    ...
    def addCourseDataFromFile(self,path):
        path_file=open(path,'r')
        lines=path_file.readlines()
        for line,i in zip(lines,range(len(lines))):
            if i== 0: # courseId
                self.setCourseID(line.strip())
            elif i == 1:
                self.setCourseName(line.strip())
            elif i == 2:
                self.setSemester(line.strip())
            elif i == 3:
                self.setInstructor(line.strip())
            elif i == 4:
                TAs=line.split(",")
                TAs=[i.strip() for i in TAs]
                self.setCourseTAs(TAs)
            else:
                pass
        self.addStudentsFromFile(path)
        path_file.close()
